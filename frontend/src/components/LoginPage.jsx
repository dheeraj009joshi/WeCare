...
const LoginPage = () => {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8000/api/auth/google/login?user_type=user');
          ...
          <div className="p-8 md:p-10">
            ...
            <motion.div className="mt-6" variants={item}>
              <motion.button
                ...
                className="w-full flex justify-center items-center px-4 py-3 border border-gray-300 rounded-xl shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#a78bfa] transition-all"
                whileHover={{ scale: 1.01 }}
                whileTap={{ scale: 0.99 }}
              >
                {googleLoading ? (
